...
def make_properties_list(propsfilename):
    ...
    for line in propsfile:
        if line.startswith("2."):
            ...
            if "CanonicalName" in propertyinfo:
...
def check_duplicate_canonical_names():
    ...
    for entry in properties:
        ...
        if canonicalname in previous_canonical_names:
    ...
    for entry in properties:
        if "AlternateNames" in entry:
            for altname in entry["AlternateNames"].split(", "):
                ...
                if altname in previous_alternative_names:
                ...
                if altname in previous_canonical_names:
...
def check_duplicate_propids():
    ...
    for entry in properties:
        if "PropertyId" in entry:
            ...
            if propnum in previous_propids and propnum < 0x6800:
        ...
        if entry["CanonicalName"].startswith("PidName") and ("PropertyId" in entry or "PropertyLid" in entry):
...
def check_proptypes():
    ...
    for entry in properties:
        ...
        if datatypename not in knowndatatypes:
...
def check_propsets():
    ...
    for entry in properties:
        if "PropertySet" in entry:
            ...
            if propsetname not in knownpropsets:
...
def check_references():
    ...
    for entry in properties:
        ...
        elif "DefiningReference" in entry:
