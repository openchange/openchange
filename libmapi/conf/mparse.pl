...
#####################################################################
# generate named_mapitags.c file
sub mapi_nameid_interface($)
{
    my $contents = shift;
    my $line;
    my @lines;
	...
	if ($line) {
	    ...
	    if ($prop[0] ne "NULL") {
		$prop[0] = sprintf "\"%s\"", $prop[0];
	    }
	    if ($prop[2] ne "NULL") {
		$prop[2] = sprintf "\"%s\"", $prop[2]
	    }
	    mparse sprintf "{ %-40s, %-15s, %-30s, %-30s, %-15s, %-20s },", 
	    $prop[0], $prop[1], $prop[2], $prop[3], $prop[4], $prop[5];
	}
    }

    mparse sprintf "{ NULL, 0x0, NULL, PT_UNSPECIFIED, 0x0, NULL }";
    deindent;
    mparse "};";
    mparse "";
...
sub process_file($)
{
	...
	FileSave($parser, mapi_nameid_interface($contents));
    }
