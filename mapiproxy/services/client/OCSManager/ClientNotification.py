...
class ClientNotification(object):
    ...
    def setNewMailPayload(self, tokenLogin=None, newmail=None):
        ...
        if newmail is None:
            return (True, 'Missing newmail arguments')

        # Sanity checks on newmail dictionary
        if not "backend" in newmail:
            return (True, 'Missing backend parameter')
        if not "username" in newmail:
            return (True, 'Missing username parameter')
        if not "folder" in newmail:
            return (True, 'Missing folder parameter')
        if not "msgid" in newmail:
            return (True, 'Missing msgid parameter')
