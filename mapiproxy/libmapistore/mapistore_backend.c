...
int mapistore_backend_folder_move_copy_messages(struct backend_context *bctx, void *source_folder, uint32_t mid_count, uint64_t *source_mids, void *target_folder, uint64_t *target_mids, uint8_t want_copy)
{
        return bctx->backend->folder.move_copy_messages(source_folder, mid_count, source_mids, target_folder, target_mids, want_copy);
}
