...
class TestMAPIStoreDB(unittest.TestCase):
	...
	def test_folder_context(self):
		...
		ipm_subtree = self.MAPIStore.get_folder_identifier(context_id=context_id, index=mapistore.MDB_IPM_SUBTREE, uri=None)
		self.assertNotEqual(ipm_subtree, 0)
		...
		retval = self.MAPIStore.opendir(context_id=context_id, parent_fid=ipm_subtree, fid=sync_fid)
		self.assertEqual(retval, 0)
