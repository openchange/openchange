                                   ...
                                   uint32_t *context_id)
{
	...
	if (namespace[1] && namespace[1] == '/' &&
		...
		namespace[3] = '\0';
		backend_ctx = mapistore_backend_create_context((TALLOC_CTX *)mstore_ctx, namespace_start, backend_uri, fid);
		if (!backend_ctx) {
...
/**
   \details Set an attribute from a stream fd
   ...
   \return MAPISTORE_SUCCESS on success, otherwise MAPISTORE errors
 */
_PUBLIC_ int mapistore_set_property_from_fd(struct mapistore_context *mstore_ctx,
					    uint32_t context_id,
					    uint64_t fmid,
					    uint8_t type,
					    uint32_t property,
					    int fd)
{
	...
	ret = mapistore_backend_set_property_from_fd(backend_ctx, fmid, type, property, fd);

	return !ret ? MAPISTORE_SUCCESS : MAPISTORE_ERROR;
}

/**
   \details Get a (streamable) attribute and put in in a file
   ...
   \return MAPISTORE_SUCCESS on success, otherwise MAPISTORE errors
 */
_PUBLIC_ int mapistore_get_property_into_fd(struct mapistore_context *mstore_ctx,
					    uint32_t context_id,
					    uint64_t fmid,
					    uint8_t type,
					    uint32_t property,
					    int fd)
{
	...
	/* Step 2. Call backend operation */
	ret = mapistore_backend_get_property_into_fd(backend_ctx, fmid, type, property, fd);
