...
/**
   \details Parse FXtransfer PT_MV_BINARY property value which does
   not follow autogenerated NDR code when the context is the Fast
   Transfer Buffer

   \param ndr_pull the NDR to get the property value from

   \param prop_value the BinaryArray_r pointer where the parsed
   property is stored

   \return the parsed result in ndr_err_code
*/
_PUBLIC_ enum ndr_err_code ndr_pull_fxtransfer_BinaryArray_r(struct ndr_pull *ndr, struct BinaryArray_r *r)
{
	uint32_t cntr_lpBinaryArray_0;
	{
		uint32_t _flags_save_STRUCT = ndr->flags;
		ndr_set_flags(&ndr->flags, LIBNDR_FLAG_NOALIGN);
		NDR_CHECK(ndr_pull_uint32(ndr, NDR_SCALARS, &r->cValues));
		if (r->cValues > 100000) {
			return ndr_pull_error(ndr, NDR_ERR_RANGE, "value out of range");
		}
		NDR_PULL_ALLOC_N(ndr, r->lpbin, r->cValues);
		if (r->lpbin) {
			NDR_PULL_SET_MEM_CTX(ndr, r->lpbin, 0);
			for (cntr_lpBinaryArray_0 = 0; cntr_lpBinaryArray_0 < r->cValues; cntr_lpBinaryArray_0++) {
				struct Binary_r *el = &r->lpbin[cntr_lpBinaryArray_0];
				NDR_CHECK(ndr_pull_uint32(ndr, NDR_SCALARS, &el->cb));
				NDR_PULL_ALLOC_N(ndr, el->lpb, el->cb);
				NDR_CHECK(ndr_pull_array_uint8(ndr, NDR_SCALARS, el->lpb, el->cb));
			}
		}
                               ...
                               struct ndr_pull *ndr_pull, uint32_t element)
{
	...
	struct BinaryArray_r PtypMvBinary;
	...
	case PT_BINARY:
		...
		break;
        case PT_MV_BINARY:
                NDR_CHECK(ndr_pull_fxtransfer_BinaryArray_r(ndr_pull, &PtypMvBinary));
                ndr_print_BinaryArray_r(ndr, propValue, &PtypMvBinary);
                break;
	default:
	{
