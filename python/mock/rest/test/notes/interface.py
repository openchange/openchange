...
class NotesInterfaceTestCase(test.MockApiBaseTestCase):
    """Basic tests for Notes module interface"""
    ...
    def _create_test_msg(self, parent_id=1, subject='Message ', body='Message body'):
        ...
        return self.post_req('/notes/', data)
    ...
    def test_get(self):
        ...
        status, text, headers = self._create_test_msg(subject='notes tget', body='notes tget body')
        item = self._to_json_ret(text)
        # fetch the message
        path = '/notes/%d/?properties=id,type,PidTagSubject,PidTagBody' % item['id']
    ...
    def test_update(self):
        ...
        # update the event
        data = {
            'PidTagBody': 'Updated notes body'
        }
        path = '/notes/%d/' % item['id']
    ...
    def test_attachment_get(self):
        ...
        # fetch the message
        path = '/notes/%d/attachments?properties=PidTagDisplayName,id' % msg_item['id']
        status, text, headers = self.get_req(path)
        self.assertEqual(status, 200)
        res = self._to_json_ret(text)
