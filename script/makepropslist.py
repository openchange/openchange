...
def make_mapi_named_properties_file():
	...
	for key in sorted(knownpropsets):
		...
		oleguid_ldif = "dn: CN=%s,CN=default\n"                 \
			       "cn: %s\n"				\
			       "name: %s\n"				\
			       "oleguid: %s\n\n" % (cn, cn, str(key), cn)
	...
	for line in sortednamedprops:
		...
		if line[5] == "MNID_STRING":
				...
				int(line[2], 16), int(line[4], 16), line[3])
		else:
                        named_props_ldif = "dn: CN=%s,CN=%s,CN=default\n"                               \
					   "objectClass: MNID_ID\n"					\
					   "cn: %s\n"							\
				...
				int(line[2], 16), int(line[4], 16), line[1])
		
		content += named_props_ldif

                if line[0]:
                        odbcontent += "\t{ 0x%8x, \"%s\" },\n" % (increment << 16 | int(line[4], 16), line[0])
	...
	f = open('setup/mapistore/mapistore_namedprops.ldif', 'w')
	f.write(content)
	f.close()

        # write named properties lookup for openchangedb
        f = open('mapiproxy/libmapiproxy/openchangedb_named_properties.c', 'w')
        f.write("""
/* Automatically generated by script/makepropslist.py. Do not edit */
#include "mapiproxy/dcesrv_mapiproxy.h"
